...
/**
 * Check if SMS contains unicode characters
 */
...
export function calculateSmsParts(text: string): {
  ...
  if (totalChars <= singleLimit) {
    return { parts: 1, charsPerPart: singleLimit, totalChars };
  }

  const parts = Math.ceil(totalChars / multiLimit);
  return { parts, charsPerPart: multiLimit, totalChars };
}
...
/**
 * Extract tokens from a template string
 */
